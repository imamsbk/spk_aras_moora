...
<div class="row-fluid">
  ...
  <div class="row">
    ...
    <div class="span11">
      <div class="box">
          <div class="box-header"> <span class="icon"><i class="icon-th"></i></span>
            <h5>Step 2 -  NORMALISASI MATRIKS</h5>
          </div>
...
///$tc1 = $cost1 + $cost0;
        $this->db->select_sum('c1');
            $tott1 = $this->db->get('tabel_kriteria')->row();
            $tc1 = ($tott1->c1)+$cost1;
    ...
    <div class="span11">
      <div class="box">
          <div class="box-header"> <span class="icon"><i class="icon-th"></i></span>
            <h5><i>Step 3 -  NORMALISASI TERBOBOT</small></h5>
          </div>
          <div class="box-body">
            <table class="table table-bordered data-table">
      <?php 
      ...
      // Hapus semua data sebelum memasukkan yang baru
        ...
        $query = $this->db->query("SELECT * from tabel_kriteria inner join tabel_siswa on tabel_kriteria.nisn=tabel_siswa.nisn")->result();
          ...
          <tbody>
              <?php 
                if (isset($_POST['simpan'])) { 
        $kriteria = $this->db->query("SELECT * from tabel_kriteria JOIN tabel_siswa on tabel_kriteria.nisn=tabel_siswa.nisn")->result();
        $nomor = 0;          
        foreach($kriteria as $row){ 
